...
def pattern(x, y, num = 3): # generates all vertices of equilateral traingle based on current leader position
...
def who_goes_where(current, vertices, needed, num=3): # each parameter contains 3 tuples of form (x, y)
    bots_dist = []
...
def new_goal(x, y, order):
    coords = pattern(x, y)
    where_to_go = []
    for i in range(len(order)):
    	where_to_go.append(coords[order[i]])
